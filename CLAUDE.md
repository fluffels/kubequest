...
| [`src/content/loader.ts`](src/content/loader.ts) | pure Domäne | Content-as-Data-Loader (#348, ADR 0004): lädt NPC-Stammdaten + Smalltalk aus `src/content/data/*.json` und validiert sie zur Laufzeit gegen ein handgeschriebenes Schema (wirft `ContentValidationError`, kein Zod – zero Runtime-Dep). Phaser-frei, unit-getestet (`test/loader.test.ts`). Erster Baustein zum Ablösen von hartcodiertem Content-TS |
