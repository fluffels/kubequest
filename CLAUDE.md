...
| [`src/content/data/`](src/content/data/) | Daten | Content-as-Data-Quelle (#348): `npcs.json`, `smalltalk.json` + **Quests pro Region/Geber** in `data/quests/<giver>.json` (eine Datei je NPC, wie Stardew) plus `data/quest-order.json` (die load-bearing Spielreihenfolge, `questIdx`). Vom Loader geladen + validiert, von Vite gebündelt; kein Runtime-`fetch`, Offline-Build bleibt heil |
