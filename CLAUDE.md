...
| [`src/content/loader.ts`](src/content/loader.ts) | pure Domäne | Content-as-Data-Loader (#348/#352, ADR 0004): lädt **NPCs, Smalltalk, alle 40 Quests und die Befehls-Karten** aus `src/content/data/*.json` und validiert sie zur Laufzeit gegen ein handgeschriebenes Schema (wirft `ContentValidationError`, kein Zod – zero Runtime-Dep). Kompiliert `accept` (String-Pattern → `RegExp`, für Quests UND Befehls-Karten) und löst `check`-Keys → Funktionen aus `checks.ts` auf; gibt `Quest[]`/`CmdCard[]` in der gewohnten Laufzeit-Form zurück. Phaser-frei, unit-getestet (`test/loader.test.ts`) |
...
| [`src/content/data/`](src/content/data/) | Daten | Content-as-Data-Quelle (#348/#349/#352): `npcs.json` (NPC-Identität), `smalltalk.json`, `entities.json` (NPC-Standplätze je Karte, #349) + **Quests pro Region/Geber** in `data/quests/<giver>.json` (eine Datei je NPC, wie Stardew) plus `data/quest-order.json` (die load-bearing Spielreihenfolge, `questIdx`) + **Befehls-Karten pro Geber** in `data/cmdcards/<giver>.json` (Spaced-Repetition-Drills, #352; `chapter` zeigt auf die Quest-ID). Vom Loader geladen + validiert, von Vite gebündelt; kein Runtime-`fetch`, Offline-Build bleibt heil |
