...
| [`src/sim/util.ts`](src/sim/util.ts) | pure Domäne | Geteilte, pure Sim-Helfer für die Befehls-Module des sim.ts-Splits (#373 ff.): Zufalls-IDs (`randSuffix`) + monospace-Tabellen-Ausgabe (`pad`/`table`), wie sie `docker ps`/`kubectl get` brauchen. Zustandslos, von `sim.ts` UND den Befehls-Modulen importiert (vermeidet einen Rückimport-Zyklus) |
