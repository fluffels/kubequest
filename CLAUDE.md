...
| [`src/content/data/`](src/content/data/) | Daten | Content-as-Data-Quelle (#348/#349/#352): `npcs.json` (NPC-Identität), `smalltalk.json`, `entities.json` (NPC-Standplätze je Karte, #349) + **Quests pro Region/Geber** in `data/quests/<giver>.json` (eine Datei je NPC, wie Stardew) plus `data/quest-order.json` (die load-bearing Spielreihenfolge, `questIdx`) + **Befehls-Karten pro Geber** in `data/cmdcards/<giver>.json` (Spaced-Repetition-Drills, #352; `chapter` zeigt auf die Quest-ID) + **Quiz-Karteikarten pro Thema** in `data/crabquiz/<thema>.json` (Krabbe Kralle, Multiple Choice, #368; nach Wissensgebiet statt Geber, da Themen wie RBAC vor ihrer Region existieren). Vom Loader geladen + validiert, von Vite gebündelt; kein Runtime-`fetch`, Offline-Build bleibt heil |
