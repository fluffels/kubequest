...
| [`src/sim/git.ts`](src/sim/git.ts) | pure Domäne | git-Befehlsfamilie (#377, sim.ts-Split 6/7 aus #346): `gitCommand(host, …)` (init/status/add/commit/log/branch/checkout/merge/push/fetch/pull) + der git-eigene Helfer für unversionierte Dateien (`gitUntracked`), aus `sim.ts` als freie Funktionen ausgelagert. Bekommt die Sim-Instanz über das schmale `GitHost`-Interface (`extends ClusterState` + die genutzten Sim-Helfer `_err`/`_suggest` und die CI-Pipeline `_runPipeline`); kein Import-Zyklus. Die CI-Pipeline-Maschinerie (`_runPipeline`) bleibt bewusst bei `glab`/CI in `sim.ts` — `git push` stößt sie nur an. Phaser-frei, abgedeckt durch die git-Tests in `test/sim.test.ts` (über `exec`) |
