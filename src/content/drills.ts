...
export const DRILLS: Record<string, (sim: Sim) => DrillTask> = {
  ...
  "docker-run-named": sim => {
    ...
    return { text: "Starte aus <code>" + img + "</code> einen Container im Hintergrund mit dem Namen <code>" + name + "</code>.", accept: [new RegExp("^docker\\s+run\\s+(?=.*-d)(?=.*--name\\s+" + name + ").*" + img + "(:\\S+)?$")], solution: "docker run -d --name " + name + " " + img, hint: "Muster: docker run -d --name <name> <image>", why: "Reihenfolge: erst die Optionen (-d für Hintergrund, --name <name>), das Image steht ganz zuletzt – Muster: docker run -d --name <name> <image>." };
  },
  ...
  "docker-build": sim => {
    ...
    return { text: "Bau aus dem <code>Dockerfile</code> ein eigenes Image <code>" + name + ":" + tag + "</code> (Punkt am Ende!).", accept: [new RegExp("^docker\\s+build\\s+-t\\s+" + name + ":" + tag.replace(/\./g, "\\.") + "\\s+\\.$")], solution: "docker build -t " + name + ":" + tag + " .", hint: "Muster: docker build -t <name>:<tag> .", why: "build schichtet aus dem Dockerfile ein eigenes Image; -t gibt den Namen, und der Punkt am Ende ist der Baukontext (der aktuelle Ordner)." };
  },
  "docker-tag": sim => {
    ensureDockerfile(sim);
    const name = pick(BUILD_NAMES);
