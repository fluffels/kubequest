...
Kurzfassung: **Genau EIN** offenes Issue, das **nicht** schon bearbeitet wird (kein Assignee/Branch/Worktree) — immer die **niedrigste freie Nummer**, nicht nach Inhalt/Prio aussuchen. **Claimen per `gh issue edit <nr> --add-assignee @me` und mit `gh issue view <nr>` verifizieren ist Pflicht und blockierend** — ohne bestätigte Zuweisung kein Implementieren. Dann **eigener `git worktree`** (Pflicht, sonst Kollision mit Parallel-Agenten), umsetzen, `npm test` + `npm run typecheck` grün, im Browser verifizieren, committen mit `(#<nr>)`, **über das Remote nach `main` integrieren und pushen** (in kubequest freigegeben), Issue schließen **und Schließung mit `gh issue view` verifizieren**, Worktree+Branch entfernen **und verifizieren**.
