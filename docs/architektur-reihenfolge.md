...
|---|--------|--------------|---------------------------|
| 1 | **#384** | Observability-Familie aus `sim.ts` nach `sim/observability.ts` auslagern (~100 Z.) | Folge zum sim.ts-Split (#346). Nach den 7 Befehlsfamilien-Splits (#372–#378 ✓) bleibt sim.ts ~786 Z.; Observability (#109/#110) ist eine kohärente Nicht-Befehls-Familie, die mit dem Inhalt wächst. **Vor #383** (Code-Split vor Test-Split). ⚠️ `podMetrics`/`nodeMetrics`/`alerts` sind öffentliche API, die `kubectl get/top` liest. |
| 2 | **#385** | glab/CI-Familie (`_glab` + `_runPipeline`) aus `sim.ts` nach `sim/glab.ts` auslagern (~50 Z.) | Folge zum sim.ts-Split (#346). Letzte Nicht-Befehls-Familie in sim.ts. **Vor #383.** ⚠️ `runPipeline` wird seit #377 von `git push` (sim/git.ts) angestoßen → wandert mit nach glab.ts, git.ts importiert es dann direkt (raus aus `GitHost`). |
| 3 | **#383** | `test/sim.test.ts` entlang der `sim/`-Module aufteilen (Test-Split zu #346) | **Nach #378 + #384 + #385** — erst wenn der ganze Code-Split fertig ist und die Modulgrenzen final stehen. Befund aus #374: `test/sim.test.ts` (1408 Z.) ist der Test-Monolith zum Code-Split; gespiegelte Dateien (kubectl/docker/helm/…), gleiche Test-Anzahl, Red-Green je Block. |
