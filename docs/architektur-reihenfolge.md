...
|---|--------|--------------|---------------------------|
| 1 | **#354** | Sprechende Quest-IDs (`harbor-first-crate` statt `q2b`) | Repo-weiter Rename + Save-Migration. Setzt auf die **ID-basierte Save** aus #353 (erledigt) auf → jetzt dran. |
| 2 | **#327** | Quests mit Thema/Kapitel (Datenmodell + Gruppierung) | Selbstpflege-Test, Grundlage fürs Logbuch-Accordion. Niedriges Risiko, foundational. |
...
| 13 | **#352** ⚠️ | CMD_CARDS auf Content-as-Data migrieren (prio:mittel) | **Gekoppelt an den laufenden ADR-0004-Umbau.** Gehört auf Branch `umbau/skalierungs-fundament` (nicht `main`); wird dort geschlossen, sobald fertig (#350 noch offen; #349 erledigt). Erst sinnvoll, wenn der Umbau-Branch bearbeitet wird. |
| 14 | **#357** ⚠️ | Entity-Registry auf Objekte/Interaktables erweitern (Folge zu #349) | **Umbau-/ADR-0004-gekoppelt:** baut auf der Entity-Registry aus #349 auf, die nur auf `umbau/skalierungs-fundament` liegt → dort umsetzen, nicht `main`. Kein Blocker (reine Skalierungs-Verbesserung) — erst sinnvoll, wenn ein Bereich viele platzierte Objekte/Trigger bekommt. |
